		...
		efi_physical_address_t efi_free_memory_bottom, size_t efi_free_memory_length)
{
	...
	if ((efi_free_memory_bottom == 0) ||
			(efi_free_memory_length == 0)) {
		return NULL;
	}
		...
		uint64_t resource_length)
{
	...
	if (rd_hop == NULL) {
		dlog_error("No space for creating resource descriptor type hob...\n");
		return -12;
	}
	...
	memset_s(&rd_hop->owner, sizeof(struct efi_guid), 0, sizeof(struct efi_guid));
		...
		struct efi_guid *guid, uint16_t data_length, void **data)
{
	...
	if (guid_hob == NULL) {
		dlog_error("No space for creating guid type hob...\n");
		return -12;
	}

	memcpy_s(&guid_hob->name, sizeof(struct efi_guid), guid, sizeof(struct efi_guid));
		...
		efi_physical_address_t base_addr, uint64_t size)
{
	...
	if (fv_hob == NULL) {
		dlog_error("No space for creating fv type hob...\n");
		return -12;
	}
