...
static void ffa_boot_info_header_init(struct ffa_boot_info_header *header,
				      size_t blob_size, enum ffa_version vm_ffa_version)
{
	assert(header != NULL);
	assert(blob_size != 0U);
...
bool ffa_boot_info_node(struct fdt_node *boot_info_node,
			struct partition_pkg *pkg, enum ffa_version vm_ffa_version)
{
	struct memiter data;
	 ...
	 */
	ffa_boot_info_header_init(boot_info_header, boot_info_size, vm_ffa_version);
